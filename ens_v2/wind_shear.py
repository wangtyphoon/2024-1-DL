...
def plot_wind_shear(member_name, shear_magnitudes, shear_directions):
    ...
    plt.savefig(f"../route/20170909/png/shear/0/Typhoon shear - Member {member_name}.png")
    # 顯示圖形
    plt.tight_layout()
    plt.show()
...
def cluster_wind_shear(df_path, ds_path, cluster_list, min_distance_km=350, max_distance_km=850):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("../route/20170909/cluster_route.png")
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("../route/20170909/cluster_shear.png")
    plt.show()
